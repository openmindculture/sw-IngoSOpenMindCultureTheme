...
If necessary, delete or add conflicting database entries, especially after migrating between partially incompatible versions.
...
Deactivate "hidden" (system) extensions:

- `mysql -u root -proot -h 127.0.0.1 -e "UPDATE app SET active = 0 WHERE name IN ('SwagCopilot', 'SwagPreviewGenerator', 'SwagAnalytics', 'Swag3DModelPipeline');" shopware`

Ensure media permissions (and location):
...
By default, the Storefront SCSS included everything. This is no longer true in Shopware 6.7.

You can disable specific Bootstrap features using flags in your `overrides.scss`.
...
##### Unregister JS Plugins
   ...
   Likewise, Shopware used to register many JS plugins (e.g., CookiePermission, SearchWidget) by default. 
   You can deregister them in your theme's entry point to prevent them from initializing and potentially save execution time,
   but that should no longer be necessary since Shopware 6.7.
